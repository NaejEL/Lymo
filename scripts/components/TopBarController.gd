...
# TopBarController.gd  
# Main application toolbar with file operations and playback controls
...
func _populate_display_selector() -> void:
	...
	for i in range(screen_count):
		...
		display_name += " - " + str(screen_rect.size.x) + "x" + str(screen_rect.size.y)
...
func _setup_grid_settings_popup() -> void:
	...
	if transform_handles_enabled_checkbox:
		transform_handles_enabled_checkbox.button_pressed = false  # Default off
